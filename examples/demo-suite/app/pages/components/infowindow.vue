<template>
  <div>
    ...
    <UCard class="mb-6"><template #header><h2 class="font-semibold">MaptalksMarkerInfoWindow · 每个 Marker 独立信息框 · 自定义关闭</h2><UBadge color="primary" variant="subtle">组件</UBadge></template>
      <p class="text-sm text-muted mb-2"><code>&lt;MaptalksMarkerInfoWindow&gt;</code> 在 <code>&lt;MaptalksMarker&gt;</code> 内，<code>:auto-open-on="'' "</code> 禁用内置自动打开，<code>@click</code> 手动 <code>open()+bindCloseBtn()</code>。</p>
