      ...
      <MaptalksMap ref="mapCmp3" :center="center" :zoom="13" class="relative rounded border border-default overflow-hidden" style="height: 400px">
        ...
        <MaptalksVectorLayer>
          <MaptalksMarker :coordinates="[121.47, 31.23]" :symbol="{ markerType: 'ellipse', markerFill: '#2563eb', markerWidth: 24, markerHeight: 24 }">
            <MaptalksMarkerInfoWindow ref="miwA" title="" :custom="true" @open="cmpOpenTime = Date.now(); cmpOpenLabel = '东门店 A'; bindCloseBtn(miwA)" @close="cmpOpenLabel = ''">
              <div style="min-width:160px;border-radius:4px;overflow:hidden;box-shadow:0 1px 6px rgba(0,0,0,0.12)">
                <div style="background:#2563eb;color:#fff;padding:4px 10px;font-size:13px;font-weight:600;display:flex;justify-content:space-between;align-items:center">
                  <span>东门店 A</span>
                  <span class="mt-miw-close" style="cursor:pointer;font-size:16px;line-height:1">×</span>
                </div>
          ...
          <MaptalksMarker :coordinates="[121.5, 31.24]" :symbol="{ markerType: 'ellipse', markerFill: '#dc2626', markerWidth: 24, markerHeight: 24 }">
            <MaptalksMarkerInfoWindow ref="miwB" title="" :custom="true" @open="cmpOpenTime = Date.now(); cmpOpenLabel = '西门店 B'; bindCloseBtn(miwB)" @close="cmpOpenLabel = ''">
              <div style="min-width:160px;border-radius:4px;overflow:hidden;box-shadow:0 1px 6px rgba(0,0,0,0.12)">
          ...
          <MaptalksMarker :coordinates="[121.52, 31.22]" :symbol="{ markerType: 'ellipse', markerFill: '#16a34a', markerWidth: 24, markerHeight: 24 }">
            <MaptalksMarkerInfoWindow ref="miwC" title="" :custom="true" @open="cmpOpenTime = Date.now(); cmpOpenLabel = '南门店 C'; bindCloseBtn(miwC)" @close="cmpOpenLabel = ''">
              <div style="min-width:160px;border-radius:4px;overflow:hidden;box-shadow:0 1px 6px rgba(0,0,0,0.12)">
...
/** 参考 composable 页 bindCloseBtn——addEventListener，并处理 innerHTML 导致的两处按钮问题 */
function bindCloseBtn(miw: typeof miwA) {
  setTimeout(() => {
    // innerHTML 让关闭按钮在隐藏 wrapper + 面板两处 DOM，找可见的那个（offsetParent !== null）
    const btns = document.querySelectorAll('.mt-miw-close');
    for (const el of btns) {
      const btn = el as HTMLElement;
      if (btn.offsetParent !== null) {
        btn.addEventListener('click', () => { miw.value?.close(); cmpOpenLabel.value = ''; }, { once: true });
        return;
      }
    }
  }, 50);
