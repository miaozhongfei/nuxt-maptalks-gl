<template>
  <div>
    ...
    <p class="text-muted mb-6">
      ...
      composable 接入：<code>mc.value?.map</code> 拿到地图实例后传给各 composable。
    </p>
    ...
    <UCard class="mb-6">
      ...
      <!-- 地图外壳用组件声明式创建 -->
      <MaptalksMap ref="mc" :center="center" :zoom="12" class="relative rounded border border-default overflow-hidden" style="height: 500px" baseLayer="osm">
      </MaptalksMap>
      <template #footer>
...
const mixMap = computed(() => toValue(mc.value?.map) ?? null)
...
const status = computed(() => (mixMap.value ? '地图已创建（组件壳 + composable）' : '加载中…'))
</script>
