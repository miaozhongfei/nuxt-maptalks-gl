<template>
  <div>
        ...
        <template #footer
          ><div class="flex gap-2 items-center">
            ...
            ><UButton
              ...
              "
              >30%</UButton
            ><span class="text-sm text-muted">当前：{{ Math.round(opacity * 100) }}%</span
      ...
      <UCard>
        ...
        <template #footer
          ><div class="flex gap-2">
            <UButton size="sm" @click="toggleDarkLayer()">{{
              dark ? '恢复亮色瓦片' : '切换暗色瓦片'
            }}</UButton
            ><span class="text-sm text-muted"
              >原生 <code>layer.config()</code> / <code>layer.setOpacity()</code></span
    ...
    <UCard class="mt-4">
      ...
      />
      <template #footer
        ><span class="text-sm text-muted">native new CanvasLayer() 自定义绘制。</span
...
watch(
  ...
  async (m) => {
    ...
    try {
      ...
      cl.addTo(m as never);
      const UIMarkerCtor = (mt.ui as Record<string, unknown>).UIMarker as new (
        coords: unknown,
