  ...
  <div>
    ...
    <div class="grid grid-cols-2 gap-4">
      <UCard>
        ...
        <template #footer><div class="flex gap-2"><UButton size="sm" color="primary" @click="showCustomIW()">显示</UButton><span class="text-sm text-muted">useMaptalksInfoWindow + 自定义 HTML</span><span class="text-xs text-muted">{{ status1 }}</span></div></template>
      </UCard>
      ...
      <UCard>
        ...
        <template #footer><span class="text-sm text-muted">原生 marker.setInfoWindow() 直调，点击 Marker 弹出。</span><span class="text-xs text-muted ml-2">{{ status2 }}</span></template>
      </UCard>
    </div>
    ...
    <UCard class="mt-4">
      ...
      <template #footer><span class="text-sm text-muted">native UIMarker + HTML 内容锚定在地图坐标上。</span><span class="text-xs text-muted ml-2">{{ status3 }}</span></template>
    </UCard>
  </div>
</template>
