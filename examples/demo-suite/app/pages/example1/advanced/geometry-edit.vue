  ...
  <div>
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <UCard>
        ...
        <template #footer><div class="flex gap-2"><UButton size="sm" color="primary" @click="toggleEdit()">切换编辑模式</UButton><span class="text-sm text-muted">原生 geometry.startEdit() / endEdit()</span><span class="text-xs text-muted">{{ status2 }}</span></div></template>
      </UCard>
    </div>
    ...
    <UCard class="mt-4">
      ...
      <template #footer><span class="text-sm text-muted">原生 new Marker({ draggable: true }) 构造器参数实现拖拽。</span><span class="text-xs text-muted ml-2">{{ status3 }}</span></template>
    </UCard>
  </div>
</template>
...
const status1 = computed(() => (ready1.value ? '地图已创建（可绘制）' : '加载中…'))
const status2 = computed(() => (ready2.value ? '地图已创建（可编辑）' : '加载中…'))
const status3 = computed(() => (ready3.value ? '地图已创建（可拖拽）' : '加载中…'))
</script>
