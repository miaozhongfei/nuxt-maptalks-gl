<template>
  <div>
    ...
    <p class="text-muted mb-6">演示曲线路径（ArcCurve / CubicBezierCurve）和连接线（ConnectorLine）逃生舱。</p>
...
onMounted(async () => {
  const layer = toValue(vec2);
  const mapVal = toValue(map2);
  if (!layer || !mapVal) return;
  const mt = await import('maptalks-gl');
