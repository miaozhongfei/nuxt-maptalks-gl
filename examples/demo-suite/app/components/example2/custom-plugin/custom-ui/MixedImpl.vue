<template>
  <div>
    ...
    <p class="text-sm text-muted mt-2">MaptalksMap ref + 自定义 UIComponent 子类（buildOn + getOffset 居中 + zoomend 闪烁）——ui.addTo(map).show()（对应官网 13.2）。</p>
    <p class="text-xs text-muted mt-1">{{ status }}</p>
  </div>
</template>

<script setup lang="ts">
// MaptalksMap ref 桥接：组件实例取 map 后挂载自定义 UIComponent
...
function toggleUi() {
  ...
  const dom = [...document.querySelectorAll('div')].find(d => d.textContent === 'Hello, MyUI')
  if (!dom) return
