...
// VectorLayer + Marker × 2 + ConnectorLine + ArcConnectorLine（一体工厂，构造器已建模）
useMaptalksLayer(map, (mt) => {
  ...
  layer.addGeometry(m1, m2)
  // ConnectorLine：直连线（始终显示）
  const line = new mt.ConnectorLine(m1, m2, {
    showOn: 'always',
    symbol: { lineColor: '#dc2626', lineWidth: 3 },
  })
  // ArcConnectorLine：弧线连接（始终显示，弧度 60°）
  const arc = new mt.ArcConnectorLine(m1, m2, {
    showOn: 'always',
    arcDegree: 60,
