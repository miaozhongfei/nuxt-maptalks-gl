  ...
  <div class="grid grid-cols-2 gap-3">
    <div>
      ...
      <p class="text-xs text-muted mt-1">
        旧 API + 简单 DOM：watch → setMenu({ custom: true, items: el })——放大/缩小。
      </p>
    </div>
    <div>
      <MaptalksMap
      ...
      <p class="text-xs text-muted mt-1">
        新封装 + slot：&lt;MaptalksMenu slot&gt;——放大/缩小 Vue 组件。
      </p>
    </div>
    <div>
      <MaptalksMap
    ...
    <div>
      ...
      <p class="text-xs text-muted mt-1">
        新封装 + slot：&lt;MaptalksMenu slot&gt; + UInput/UButton 双向绑定。
      </p>
    </div>
...
function flyToCoord(m: MaptalksMap | null | undefined, s: string) {
...
function doSearch4() {
  flyToCoord(toValue(mc4.value?.map), searchText.value)
}

const status = computed(() => (toValue(mc1.value?.map) ? '地图已创建（自定义右键菜单可用）' : '加载中…'))
</script>
