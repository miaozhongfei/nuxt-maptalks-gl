...
const m1a = useMaptalksMarker(layer1, {
  coordinates: MARKERS[0] as [number, number],
  options: { symbol: MKR_SYM },
})
const m1b = useMaptalksMarker(layer1, {
  coordinates: MARKERS[1] as [number, number],
  options: { symbol: MKR_SYM },
})
const m1c = useMaptalksMarker(layer1, {
  coordinates: MARKERS[2] as [number, number],
  options: { symbol: MKR_SYM },
})
watch([() => toValue(map1), () => toValue(m1a.geometry)], () => {
  const g = toValue(m1a.geometry)
...
watch([() => toValue(map1), () => toValue(m1b.geometry)], () => {
  const g = toValue(m1b.geometry)
  const m = toValue(map1)
  if (g && m)
    g.setMenu?.({
      width: 160,
...
watch([() => toValue(map1), () => toValue(m1c.geometry)], () => {
  const g = toValue(m1c.geometry)
  const m = toValue(map1)
  if (g && m)
    g.setMenu?.({
      width: 160,
...
const m2a = useMaptalksMarker(layer2, {
  coordinates: MARKERS[0] as [number, number],
  options: { symbol: MKR_SYM },
})
const m2b = useMaptalksMarker(layer2, {
  coordinates: MARKERS[1] as [number, number],
  options: { symbol: MKR_SYM },
})
const m2c = useMaptalksMarker(layer2, {
  coordinates: MARKERS[2] as [number, number],
  options: { symbol: MKR_SYM },
})
useMaptalksGeometryMenu(m2a.geometry, {
  options: {
    width: 160,
    items: [
...
useMaptalksGeometryMenu(m2b.geometry, {
  options: {
    width: 160,
    items: [
      { item: '放大', click: () => toValue(map2)?.zoomIn() },
...
useMaptalksGeometryMenu(m2c.geometry, {
  options: {
    width: 160,
    items: [
      { item: '放大', click: () => toValue(map2)?.zoomIn() },
...
const m3a = useMaptalksMarker(layer3, {
  coordinates: MARKERS[0] as [number, number],
  options: { symbol: MKR_SYM },
})
const m3b = useMaptalksMarker(layer3, {
  coordinates: MARKERS[1] as [number, number],
  options: { symbol: MKR_SYM },
})
const m3c = useMaptalksMarker(layer3, {
  coordinates: MARKERS[2] as [number, number],
  options: { symbol: MKR_SYM },
})
function setOldC(g: MaptalksGeometry | null, m: MaptalksMap | null) {
  if (g && m)
...
watch([() => toValue(map3), () => toValue(m3a.geometry)], () =>
  setOldC(toValue(m3a.geometry), toValue(map3)),
)
watch([() => toValue(map3), () => toValue(m3b.geometry)], () =>
  setOldC(toValue(m3b.geometry), toValue(map3)),
)
watch([() => toValue(map3), () => toValue(m3c.geometry)], () =>
  setOldC(toValue(m3c.geometry), toValue(map3)),
)
...
const m4a = useMaptalksMarker(layer4, {
  coordinates: MARKERS[0] as [number, number],
  options: { symbol: MKR_SYM },
})
const m4b = useMaptalksMarker(layer4, {
  coordinates: MARKERS[1] as [number, number],
  options: { symbol: MKR_SYM },
})
const m4c = useMaptalksMarker(layer4, {
  coordinates: MARKERS[2] as [number, number],
  options: { symbol: MKR_SYM },
})
useMaptalksGeometryMenu(m4a.geometry, {
  options: {
    custom: true,
    items: customEl(
      () => toValue(map4)?.zoomIn(),
...
useMaptalksGeometryMenu(m4b.geometry, {
  options: {
    custom: true,
    items: customEl(
      () => toValue(map4)?.zoomIn(),
...
useMaptalksGeometryMenu(m4c.geometry, {
  options: {
    custom: true,
    items: customEl(
      () => toValue(map4)?.zoomIn(),
...
const status = computed(() => (map1.value ? '地图已创建（图形右键菜单可用）' : '加载中…'))
</script>
