<template>
  <div class="grid grid-cols-2 gap-3">
    ...
    <div>
      ...
      <p class="text-xs text-muted mt-1">旧 API：watch → geo.setMenu({ custom: true })——自定义 HTML。</p>
    </div>
    <div>
      <div ref="el4" class="relative rounded border border-default overflow-hidden" style="height: 280px" />
      <p class="text-xs text-muted mt-1">新封装：new mt.ui.Menu({ custom: true }) + contextmenu——自定义 HTML。</p>
    </div>
  </div>
</template>
...
const MENU_ITEMS = [
  { item: '放大', click: (m: any) => { m.zoomIn() } },
  { item: '缩小', click: (m: any) => { m.zoomOut() } },
]
...
async function setupMarkers(m: maptalks.Map, mode: 'old-s' | 'new-s' | 'old-c' | 'new-c') {
  const mt = await import('maptalks-gl')
  const layer = new mt.VectorLayer('v').addTo(m)
  for (const coord of MARKERS) {
    const marker = new mt.Marker(coord, { symbol: { markerType: 'ellipse', markerFill: '#f59e0b', markerWidth: 14, markerHeight: 14 } })
    marker.addTo(layer)
    if (mode === 'old-s') {
      marker.setMenu({ width: 160, items: [{ item: '放大', click: () => m.zoomIn() }, { item: '缩小', click: () => m.zoomOut() }] } as any)
    } else if (mode === 'new-s') {
      const mu = new mt.ui.Menu({ width: 160, items: [{ item: '放大', click: () => m.zoomIn() }, { item: '缩小', click: () => m.zoomOut() }] } as any)
      mu.addTo(marker as any)
      const h = (e: any) => mu.show(e.coordinate)
      marker.on('contextmenu', h)
    } else if (mode === 'old-c') {
      marker.setMenu({ custom: true, items: customEl(() => m.zoomIn(), () => m.zoomOut()) } as any)
    } else if (mode === 'new-c') {
      const mu = new mt.ui.Menu({ custom: true, items: customEl(() => m.zoomIn(), () => m.zoomOut()) } as any)
      mu.addTo(marker as any)
      const h = (e: any) => mu.show(e.coordinate)
      marker.on('contextmenu', h)
    }
...
watch(() => toValue(map3), (m) => { if (m) void setupMarkers(m, 'old-c') })
...
watch(() => toValue(map4), (m) => { if (m) void setupMarkers(m, 'new-c') })
</script>
