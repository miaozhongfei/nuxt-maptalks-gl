<template>
  <div>
    ...
    <p class="text-sm text-muted mt-2">逃生舱——官网原生方式：直接改 uiMarker.options（零重建）（对应官网 10.8）。</p>
  </div>
</template>
...
let uim: { options: Record<string, unknown> } | null = null
watch(() => toValue(map), async (m) => {
  if (!m) return
  const mt = await import('maptalks-gl')
  // 锚点参照：5×5 椭圆 Marker
  const layer = new mt.VectorLayer('v').addTo(m as any)
  new mt.Marker([121.5057, 31.2453], { symbol: { markerType: 'ellipse', markerFill: '#000', markerWidth: 5, markerHeight: 5 } }).addTo(layer)
  // UIMarker：内容 + 初始对齐
