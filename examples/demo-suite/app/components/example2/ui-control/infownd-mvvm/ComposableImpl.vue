...
function buildMvvmEl(): HTMLElement | null {
  ...
  const d = document.createElement('div'); d.style.cssText = 'padding:10px;min-width:180px;font-size:14px'
  // 标题
  const h2 = document.createElement('h2'); h2.textContent = name.value; h2.style.cssText = 'margin:0 0 6px;font-size:16px'
  // 输入框：input 事件 → 更新 name → 手动同步标题
  const inp = document.createElement('input'); inp.value = name.value
  ...
  // 计数器
  const cnt = document.createElement('div'); cnt.style.cssText = 'margin-bottom:6px'
  const cntText = document.createElement('span'); cntText.textContent = `count：${count.value}`
  const btn = document.createElement('button'); btn.textContent = 'count++'
  ...
  btns.value.forEach((b) => {
    const bEl = document.createElement('button'); bEl.textContent = String(b)
