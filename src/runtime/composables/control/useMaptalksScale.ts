...
import type { MaptalksScaleControl, MaptalksControl, MaptalksEventHandler, MaptalksMap, UseMaptalksControlReturn, UseMaptalksScaleOpts } from '../../types';
 ...
 * 地图控件：比例尺（Scale）。
 *
 * @description 在 map 就绪后创建 Scale 控件并 `addTo(map)`；响应式 `options` 变化时移除旧控件并重建；
 ...
 * @param {UseMaptalksScaleOpts} [opts] - 控件选项、事件绑定与自动销毁控制
 * @returns {UseMaptalksControlReturn} `{ control, show, hide, remove }`
 *
 * @example
 * const { map } = useMaptalks(el);
 * const { control } = useMaptalksScale(map, {
 *   options: { position: 'bottom-left', metric: true },
 *   events: { positionchange: () => console.log('位置变化') },
 * });
 */
export function useMaptalksScale(
  map: MaybeRefOrGetter<MaptalksMap | null>,
  opts: UseMaptalksScaleOpts = {},
): UseMaptalksControlReturn<MaptalksScaleControl> {
  const control = shallowRef<MaptalksScaleControl | null>(null);
  let creating = false;
  ...
  async function reload() {
    ...
    try {
      ...
      const Ctor = mt.control?.Scale;
      if (typeof Ctor !== 'function')
        throw new MaptalksError('control-failed', '当前 maptalks-gl 未导出 Scale 控件');
      const ctrl = new Ctor(toValue(opts.options)) as MaptalksScaleControl;
      ctrl.addTo(m);
