<template><!-- maptalks wms layer � 纯逻辑组件 --></template>

<script setup lang="ts">
/**
 * WMS 瓦片图层组件（WMSTileLayer）。
 *
 * @description 对 `useMaptalksWMSLayer` 的声明式封装。支持命名源或内联 WMS 配置（url + options）。
 * `WMSTileLayer` 继承 `TileLayer`，额外支持 `layers`（WMS 图层名）、`format`（图片格式）、`transparent` 等 WMS 业务参数。
 * 支持 `:visible` 声明式显隐、`:events` 原生事件绑定、`defineExpose({ layer, show, hide })` 程序式控制。
 * 必须在 MaptalksMap 内使用。
 *
 * @example
 * ```vue
 * <MaptalksWMSLayer
 *   source="myWMS"
 *   :visible="true"
 *   :options="{ layers: 'topp:states', format: 'image/png', transparent: true }"
 * />
 * ```
 */
import { computed, inject, watch } from 'vue'

import { useMaptalksWMSLayer } from '../../composables/layer/useMaptalksWMSLayer'
import { MAP_KEY } from '../../core/map-context'
import type { MaptalksEventHandler, MaptalksWMSLayerOptions, MaptalksSource } from '../../types'

const props = withDefaults(
  defineProps<{
    /** 数据源：源名（字符串，按配置解析）或内联源对象 */
    source?: string | MaptalksSource
    /** 图层 id，缺省自动生成 */
    id?: string | number
    /** 透传给 WMSTileLayer 构造器的选项（含 WMS 业务参数：layers / format / transparent / version / crs 等） */
    options?: MaptalksWMSLayerOptions
    /** 组件销毁时自动移除图层，默认 true */
    autoDispose?: boolean
    /** 是否可见，默认 true */
    visible?: boolean
    /** 事件名 → 处理器（自动 on/off） */
    events?: Record<string, MaptalksEventHandler>
  }>(),
  { options: () => ({}), autoDispose: true, visible: true },
)

const map = inject(MAP_KEY)
if (!map) throw new Error('[nuxt-maptalks-gl] MaptalksWMSLayer 必须在 MaptalksMap 内使用')
const { layer, show, hide } = useMaptalksWMSLayer(map, {
  source: props.source,
  id: props.id,
  options: computed(() => props.options),
  autoDispose: props.autoDispose,
  events: props.events,
})

watch([() => props.visible, layer], ([v, l]) => { if (!l) return; if (v) show(); else hide() }, { immediate: true })

/** 暴露 layer 实例与 show/hide 方法，供 template ref 访问 */
defineExpose({ layer, show, hide })
</script>
